...
public class HttpTrigger1
{
    ...
    public async Task<HttpResponseData> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "generate-sas/{blobName}")] HttpRequestData req,
        string blobName)
    {
        ...
        if (string.IsNullOrEmpty(connectionString) || string.IsNullOrEmpty(containerName) || string.IsNullOrEmpty(keyVaultUri))
        {
            ...
            _logger.LogError($"Missing configuration - ConnectionString: {!string.IsNullOrEmpty(connectionString)}, ContainerName: {!string.IsNullOrEmpty(containerName)}, KeyVaultUri: {!string.IsNullOrEmpty(keyVaultUri)}");
